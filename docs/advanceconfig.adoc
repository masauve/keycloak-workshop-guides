...
<4> Copy this value, making sure you select everything.x
...
These jar files were build previously and for convenience stored in a GIT responsitory. We will use the operator to load these extensions into our keycloak server.
...
add the following section under the `spec:` section
...
The keycloak server should be restarting with the added configuration
