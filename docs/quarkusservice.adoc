...
Login to your SSO instance again; https://keycloak-{{ USER_ID }}-keycloak.{{ ROUTE_SUBDOMAIN }}
...
[source, shell]
----
quarkus.oidc.auth-server-url=https://keycloak-{{ USER_ID }}-keycloak.{{ ROUTE_SUBDOMAIN }}/auth/realms/quarkus <1> 
...
<1> is the authentication url that our Quarkus application will use to establish authentication.
...
Now that we have all the dependencies required. Lets look at the following imports to our `BookResource.java`
